...
pub async fn logs_handler(
    ...
    if logs_dir.exists() {
        if let Ok(entries) = fs::read_dir(logs_dir) {
                ...
                if path.exists() {
                    if let Ok(content) = fs::read_to_string(path) {
                        ...
                        for line in lines {
                            ...
                            processed_content.push_str(&format!("<div class='log-entry {}'>{}</div>", level_class, escaped));
                        }
                        log_content = processed_content;
                        found_logs = true;
                    }
                } else {
                    log_content = "<div class='log-entry log-error'>Log file not found.</div>".to_string();
                }
            } else {
         ...
         log_content = "<div class='log-entry log-error'>Logs directory not found.</div>".to_string();
